...
;(async () => {
  try {
    ...
    const host = "mongodb+srv://juank80:mongo80@mflix-nd8sf.mongodb.net/test"
    const client = await MongoClient.connect(
      host,
      { useNewUrlParser: true },
    )
    const mflix = client.db("sample_mflix")
    const m_movies = mflix.collection("movies");
    ...
    console.log(
        "\x1b[32m",
        `Coursor Found ${cursor.length} documents to update`,
      )
    ...
    const { modifiedCount } = await m_movies.bulkWrite( moviesToMigrate )
